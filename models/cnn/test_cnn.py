...
test_cnn.py
===========
Evaluate the best ResNet50 image classifier checkpoint on the MMHS150K test set.
Loads the Colab-trained checkpoint from models/checkpoints/cnn/.

Usage (from repo root):
    python -m models.cnn.test_cnn                    # binary (default)
    python -m models.cnn.test_cnn --multiclass       # 6-class
    python -m models.cnn.test_cnn --checkpoint path/to/custom.pt
...
# ─────────────────────────────────────────────────────────────────────────────
def parse_args():
    p = argparse.ArgumentParser(description="Test ResNet50 image classifier on MMHS150K")
    p.add_argument("--multiclass",  action="store_true", help="6-class instead of binary")
    p.add_argument("--checkpoint",  type=str, default=None,
                   help="Path to checkpoint file (default: models/checkpoints/cnn/cnn_binary_best.pt)")
...
def main():
    ...
    ckpt_path = Path(args.checkpoint) if args.checkpoint else CKPT_DIR / f"cnn_{task_tag}_best.pt"
    run_dir   = RESULT_DIR / task_tag
    run_dir.mkdir(parents=True, exist_ok=True)

    print("=" * 65)
    print(f"  ResNet50 Image Classifier — Test Evaluation")
