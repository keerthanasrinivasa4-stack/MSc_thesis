...
train_multimodal.py
===================
Full training pipeline for the late-fusion Multimodal classifier on MMHS150K.

Usage (from repo root):
    python -m models.multimodal.train_multimodal                  # full training
    python -m models.multimodal.train_multimodal --quick          # sanity-check
    python -m models.multimodal.train_multimodal --epochs 5
    python -m models.multimodal.train_multimodal --multiclass     # 6-class
    python -m models.multimodal.train_multimodal --help

Architecture: BERT text branch + ResNet50 image branch → late concat fusion
Outputs saved to results/multimodal/<binary|6class>/
Checkpoints  saved to models/checkpoints/multimodal/
"""
...
# ─────────────────────────────────────────────────────────────────────────────
def parse_args():
    p = argparse.ArgumentParser(description="Train Multimodal (BERT+ResNet50) on MMHS150K")
    p.add_argument("--epochs",        type=int,   default=5)
    p.add_argument("--batch_size",    type=int,   default=32,   help="Batch size (default: 32 — multimodal is memory-heavy)")
    p.add_argument("--lr",            type=float, default=2e-5, help="Learning rate (default: 2e-5)")
    p.add_argument("--warmup",        type=float, default=0.1,  help="LR warm-up fraction (default: 0.1)")
    p.add_argument("--max_len",       type=int,   default=96,   help="BERT max token length (default: 96)")
    p.add_argument("--dropout",       type=float, default=0.4,  help="Fusion dropout rate (default: 0.4)")
    ...
    p.add_argument("--quick",         action="store_true",      help="Limit to 2000/500/1000 for sanity check")
...
# ─────────────────────────────────────────────────────────────────────────────
# Custom collate — pack text dicts + image tensors + labels into batch
# ─────────────────────────────────────────────────────────────────────────────
def multimodal_collate(batch):
    text_data_list, img_list, label_list = zip(*batch)
    # Stack text fields
    text_batch = {
        k: torch.stack([d[k] for d in text_data_list])
        for k in text_data_list[0]
    }
    imgs   = torch.stack(img_list)
    labels = torch.stack(label_list)
    return text_batch, imgs, labels
...
def evaluate(model, loader, loss_fn, device):
    ...
    return avg_loss, acc, f1_macro, labels_all, preds_all
...
def plot_training_curves(history: dict, out_path: Path):
    ...
    axes[0].plot(epochs, history["train_loss"], "o-", color="#9C27B0", label="Train Loss")
    ...
    axes[1].plot(epochs, history["train_acc"], "o-", color="#9C27B0", label="Train Acc")
...
def main():
    ...
    print("=" * 65)
    print(f"  Multimodal Classifier (BERT + ResNet50) — MMHS150K")
    ...
    train_ds = MMHS150KMultimodalDataset(
        train_ids, gt, IMG_DIR, tokenizer, TRAIN_TRANSFORMS, args.max_len, binary)
    val_ds   = MMHS150KMultimodalDataset(
        val_ids,   gt, IMG_DIR, tokenizer, EVAL_TRANSFORMS,  args.max_len, binary)
    ...
    if not args.no_weighted_sampler:
        ...
        train_loader = DataLoader(
            ...
            num_workers=nw, collate_fn=multimodal_collate, pin_memory=True)
    else:
        train_loader = DataLoader(
    ...
    # ── Optimizer & scheduler ─────────────────────────────────────────────
    # Lower LR for pretrained BERT/ResNet, higher for new fusion head
    optimizer = torch.optim.AdamW([
        {"params": model.bert.parameters(),       "lr": args.lr},
        {"params": model.resnet.parameters(),     "lr": args.lr * 0.1},
        {"params": model.text_proj.parameters(),  "lr": args.lr * 5},
        {"params": model.img_proj.parameters(),   "lr": args.lr * 5},
    ...
    # ── Training loop ─────────────────────────────────────────────────────
    best_val_f1 = -1.0
    best_ckpt   = CKPT_DIR / f"multimodal_{task_tag}_best.pt"
    last_ckpt   = CKPT_DIR / f"multimodal_{task_tag}_last.pt"
    ...
    with open(run_dir / "classification_report.txt", "w") as f:
        ...
        f.write(report_str)
        f.write(f"\n\nConfusion Matrix:\n{cm}\n")
    print(f"  Saved → {run_dir / 'classification_report.txt'}")

    metrics = {
        "task": task_tag, "model": "multimodal_bert_resnet50",
        "epochs_ran": args.epochs, "best_val_f1": best_val_f1,
        "history": history,
        "test": {
